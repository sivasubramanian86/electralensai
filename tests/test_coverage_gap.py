...
def test_multimedia_service_multimodal(mock_from_pretrained, mock_genai_client):
    ...
    response = client.post("/v1/imagen/generate", json={
        "concept": "election",
...
def test_multimedia_job_flow(mock_gen_audio, mock_gen_info, mock_gen_package):
...
def test_live_websocket_full(mock_run_live, mock_queue_cls):
...
def test_agent_stream_full(mock_run_async):
...
@pytest.mark.asyncio
async def test_memory_service_real():
    from agents.memory import memory_service
    with patch("agents.memory._USE_ALLOYDB", True), \
        ...
        mock_conn.fetch.return_value = [{"id": 1, "title": "T", "description": "D", "resolution": "R", "embedding": None}]
...
def test_multimedia_service_audio(mock_storage, mock_tts):
...
def test_multimedia_service_package_real(mock_gen_content):
    from api.services.multimedia_service import multimedia_service
    mock_gen_content.return_value.text = "Mock Script"
    
    # Mock dependencies to avoid real GCP calls
    with patch.object(multimedia_service, "generate_infographic", return_value="http://img"), \
...
def test_multimedia_service_errors():
    from api.services.multimedia_service import multimedia_service
    
    # Test audio generation failure
    with patch.object(multimedia_service.tts_client, "synthesize_speech", side_effect=Exception("TTS Fail")):
...
@patch("api.routers.live_router.LiveRequestQueue")
@patch("api.routers.live_router.live_runner.run_live")
def test_live_websocket_json_error(mock_run_live, mock_queue_cls):
    mock_queue = mock_queue_cls.return_value
...
async def test_live_tools():
    ...
    with patch("api.live_tools.multimedia_service.generate_multimodal_package", return_value={"ok": True}):
...
@pytest.mark.asyncio
async def test_memory_service_local():
    from agents.memory import memory_service
...
@pytest.mark.asyncio
async def test_memory_service_exceptions():
    from agents.memory import memory_service
    with patch("agents.memory._USE_ALLOYDB", True), \
         patch("agents.memory.os.getenv", return_value="postgresql://user:pass@host/db"), \
...
def test_pubsub_service_coverage():
    from api.services.pubsub_service import PubSubService
    
    # Test initialization with project_id but publisher failure
    with patch("google.cloud.pubsub_v1.PublisherClient", side_effect=Exception("Init Fail")), \
...
@pytest.mark.asyncio
async def test_memory_internal_embed():
...
def test_analytics_service_gaps():
    from api.services.analytics_service import AnalyticsService
    
    # Test init failure for monitoring
    with patch("google.cloud.monitoring_v3.MetricServiceClient", side_effect=Exception("Init Fail")), \
         patch("os.getenv", side_effect=lambda k, d=None: "test-proj" if k=="GOOGLE_CLOUD_PROJECT" else "true" if k=="ENABLE_ANALYTICS" else d):
...
def test_cloud_logger_gaps():
    from api.services.cloud_logger import CloudLoggingService
    
    # Test init failure
    with patch("google.cloud.logging.Client", side_effect=Exception("Log Fail")), \
...
def test_multimedia_router_gaps(mock_job_service, mock_multimedia_service):
    from api.routers.multimedia_router import get_job_status, generate_multimodal_content, run_multimodal_job
...
@patch("api.routers.live_router.LiveRequestQueue")
@patch("api.routers.live_router.live_runner.run_live")
def test_live_router_gaps(mock_run_live, mock_queue_cls):
    mock_queue = mock_queue_cls.return_value
    ...
    with patch("api.routers.live_router.LiveRequestQueue", side_effect=Exception("Queue Init Fail")):
        with client.websocket_connect("/ws/session") as websocket:
            websocket.receive_json() # session_id
            # Should log error and not crash
            websocket.close()
...
def test_translation_service_gaps():
    from api.services.translation_service import TranslationService
    # Test error during TranslationServiceClient init
    with patch("google.cloud.translate_v3.TranslationServiceClient", side_effect=Exception("Trans Fail")), \
         patch("os.environ.get", return_value="test-proj"):
        service = TranslationService()
        assert service.client is None

    # Test translate_text error branch (line 48)
    with patch("api.services.translation_service.translate.TranslationServiceClient") as mock_client_cls, \
         patch("os.environ.get", return_value="test-proj"):
...
@patch("api.services.multimedia_service.client")
def test_multimedia_service_gaps(mock_genai_client):
    from api.services.multimedia_service import multimedia_service
...
def test_agent_router_gaps():
    from api.routers.agent_router import _stream_agent_response, stream_query, query
...
def test_audit_router_gaps():
    ...
    with patch("api.routers.audit_router.memory_service.get_historical_precedents", side_effect=Exception("DB Fail")):
        with pytest.raises(HTTPException) as exc:
            asyncio.run(list_precedents("voter id"))
        assert exc.value.status_code == 500

# 35. Test Live Router Final Gaps
def test_live_router_final_gaps():
    # Test websocket.close exception in cleanup (line 149-150)
    with patch("api.routers.live_router.LiveRequestQueue") as mock_q, \
...
# 36. Test Agent Router Streaming Gaps
@patch("api.routers.agent_router.runner.run_async")
def test_agent_router_streaming_gaps(mock_run_async):
    ...
    with patch("api.routers.agent_router.translation_service.translate_text", return_value="Bonjour") as mock_trans:
...
def test_translation_service_no_project():
    from api.services.translation_service import TranslationService
    with patch("os.environ.get", return_value=None), \
...
def test_imagen_router_error():
    with patch("api.routers.imagen_router.multimedia_service.generate_infographic", side_effect=Exception("Imagen Error")):
...
def test_live_router_more_gaps():
...
def test_agent_query_success(mock_run_async):
    async def mock_gen(*args, **kwargs):
...
def test_multimedia_success_branch():
    from api.services.multimedia_service import multimedia_service
    
    # Patch internal objects directly on the singleton
    with patch.object(multimedia_service, "storage_client") as mock_storage, \
...
def test_job_service_edge_cases():
    from api.services.job_service import job_service
...
def test_live_router_cleanup_exception():
...
def test_config_gaps():
    ...
    with patch("api.config.secretmanager.SecretManagerServiceClient", side_effect=Exception("SM down")):
        assert Config.get_secret("any", "default") == "default"

# 47. Test Live Tools Exception (lines 19-21)
@pytest.mark.asyncio
async def test_live_tools_exception():
    from api.live_tools import generate_inclusive_assets
    with patch("api.live_tools.multimedia_service.generate_multimodal_package", side_effect=Exception("Package error")):
        res = await generate_inclusive_assets("topic")
        assert "error" in res

# 48. Test Agent Router Translation Branch (line 202)
@patch("agents.runner.run_async")
def test_agent_query_translation_branch(mock_run_async):
    async def mock_gen(*args, **kwargs):
        mock_part = MagicMock()
