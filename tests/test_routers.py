...
class TestMultimediaRouter:
    ...
    def test_generate_multimedia(self, mock_create, client):
...
class TestImagenRouter:
    ...
    def test_generate_image(self, mock_gen, client):
    ...
    @patch("api.routers.imagen_router.multimedia_service.generate_infographic")
    def test_generate_image_error(self, mock_gen, client):
...
class TestLiveTools:
    ...
    @pytest.mark.asyncio
    async def test_generate_inclusive_assets(self, mock_gen):
    ...
    @pytest.mark.asyncio
    async def test_verify_civic_document(self, mock_mask):
...
class TestAgentRouterStreaming:
    ...
    def test_stream_query(self, mock_run_async, client):
    ...
    def test_stream_query_with_translation(self, mock_translate, mock_run_async, client):
        async def mock_iter(*args, **kwargs):
            class MockPart:
                def __init__(self, text):
                    self.text = text
    ...
    @patch("api.routers.agent_router.runner.run_async")
    def test_stream_query_error(self, mock_run_async, client):
    ...
    @patch("api.routers.agent_router.runner.run_async")
    def test_query_sync_error(self, mock_run_async, client):
        mock_run_async.side_effect = Exception("Agent failed")
