...
class TestDlpService:
    ...
    def test_mask_text_success(self, mock_dlp_client_class):
    ...
    @patch('google.cloud.dlp_v2.DlpServiceClient')
    def test_mask_text_failure(self, mock_dlp_client_class):
        mock_client = mock_dlp_client_class.return_value
...
class TestMultimediaService:
    ...
    def test_generate_infographic_success(self, mock_genai_client, mock_imagen_class, mock_tts_client, mock_storage_client):
    ...
    @patch('google.cloud.storage.Client')
    @patch('google.cloud.texttospeech.TextToSpeechClient')
    def test_generate_audio_guide_success(self, mock_tts_client_class, mock_storage_client):
        ...
        mock_blob.upload_from_string.assert_called_once_with(b"audio data", content_type="audio/mpeg")

    @patch('google.cloud.storage.Client')
    @patch('google.cloud.texttospeech.TextToSpeechClient')
    @patch('vertexai.preview.vision_models.ImageGenerationModel.from_pretrained')
    @patch('api.services.multimedia_service.client')
    def test_generate_multimodal_package(self, mock_genai_client, mock_imagen_class, mock_tts_client, mock_storage_client):
        service = MultimediaService()
