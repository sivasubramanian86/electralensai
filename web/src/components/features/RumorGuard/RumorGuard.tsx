    ...
    <div className="max-w-5xl mx-auto h-full flex flex-col space-y-8 animate-in fade-in duration-700">
      <div className="text-left space-y-2">
        ...
        <p className="text-slate-400 max-w-2xl">
          Don't let rumors cloud your decision. We help you verify viral claims against official rules and teach you how to check for yourself.
        </p>
      </div>
      ...
      <div className="relative group">
        ...
        <input
          ...
          placeholder="Paste a viral forward or claim here..."
          value={query}
        ...
        >
          {status === 'loading' || status === 'streaming' ? <Loader2 className="w-5 h-5 animate-spin" /> : 'Verify'}
        </button>
      </div>
        ...
        <div className="lg:col-span-2 flex flex-col space-y-6">
          <div className="flex-1 glass rounded-3xl border border-white/10 p-8 relative overflow-hidden flex flex-col">
            {(status !== 'idle' || output) ? (
              <div className="space-y-6 animate-in slide-in-from-left-4 duration-500 h-full flex flex-col">
                ...
                <div className="flex-1 overflow-y-auto p-5 bg-white/5 rounded-2xl border border-white/5 markdown-body text-slate-300">
                   {output ? (
                      ...
                      <div className="flex items-center gap-2 text-slate-500 italic">
                        <Loader2 className="w-4 h-4 animate-spin" /> Consulting official election guidelines...
                      </div>
                   )}
                </div>
                ...
                <div className="pt-6 border-t border-white/5 mt-auto flex justify-between items-center">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Grounded in Google Search & Official Sources</span>
                  </div>
              ...
              <div className="flex-1 flex flex-col items-center justify-center text-center space-y-4 text-slate-500">
                ...
                <div>
                  <p className="font-bold">Enter a claim to begin analysis</p>
                  <p className="text-sm max-w-xs mx-auto">We use Gemini and live Search to cross-reference with actual election law.</p>
                </div>
              </div>
            )}
          </div>
          ...
          <div className="p-4 rounded-2xl bg-amber-500/5 border border-amber-500/20 flex gap-4 items-start">
            ...
            <p className="text-xs text-amber-500/80 leading-relaxed">
              <strong>Safe Deflection:</strong> If a topic is too sensitive or results are inconclusive, we will always refer you to your local Election Authority. We prioritize accuracy over instant answers.
            </p>
          </div>
        </div>
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-3">
            {trendingMyths.map((m) => (
              ...
              >
                ...
                <p className="text-xs font-medium text-slate-300 line-clamp-2">"{m.claim}"</p>
              </button>
            ))}
          </div>
        </div>
