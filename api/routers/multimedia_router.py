...
async def run_multimodal_job(job_id: str, topic: str, language: str):
    """Background task to run the generation pipeline."""
    try:
        job_service.update_job(job_id, "processing")
        content = multimedia_service.generate_multimodal_package(
...
async def generate_multimodal_content(request: MultimediaRequest, background_tasks: BackgroundTasks) -> dict:
...
async def list_multimedia_jobs():
